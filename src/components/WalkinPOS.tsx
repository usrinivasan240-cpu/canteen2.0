    ...
    <div className="space-y-4">
      ...
      {isOnline && offlineBills.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-red-800">Syncing {offlineBills.length} offline bill(s)...</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
        ...
        <div className="xl:col-span-8 space-y-4">
          ...
          <div className="bg-white p-4 rounded-2xl border border-red-100 shadow-xs space-y-3">
            <div className="flex gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-red-50/30 hover:bg-red-50/60 focus:bg-white text-xs pl-10 pr-4 py-3 border border-red-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 font-sans transition-all"
                />
                {searchQuery && (
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-[10px] font-bold rounded-full whitespace-nowrap transition-all cursor-pointer ${
                    selectedCategory === cat
                      ...
                      : 'bg-red-50 text-amber-600 hover:bg-red-100'
                  }`}
                >
                  {cat}
                </button>
              ...
              return (
                <button
                  ...
                  className={`relative bg-white p-3 rounded-2xl border text-left transition-all cursor-pointer group ${
                    inCart
                      ? 'border-amber-400 ring-2 ring-red-200 shadow-md'
                      : stockLeft <= 0
                        ? 'border-gray-100 opacity-50 cursor-not-allowed'
                        : 'border-red-100 hover:border-red-300 hover:shadow-md active:scale-95'
                  }`}
                >
                  {inCart && (
                    <span className="absolute -top-2 -right-2 bg-amber-600 text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shadow">
                      {inCart.quantity}
                    </span>
                  )}
                  {item.imageUrl ? (
                    <div className="w-full h-20 rounded-xl bg-red-50 overflow-hidden mb-2">
                      <img src={item.imageUrl} alt={item.name} className="w-full h-full object-cover" />
                    </div>
                  ) : (
                    <div className="w-full h-20 rounded-xl bg-gradient-to-br from-red-50 to-red-100 flex items-center justify-center mb-2 text-2xl">
                      🍽️
                    </div>
                  )}
                  <p className="font-display font-bold text-[11px] text-gray-900 truncate">{item.name}</p>
                  <div className="flex items-center justify-between mt-1">
                    <span className="font-mono font-bold text-xs text-red-800">₹{item.price}</span>
        ...
        <div className="xl:col-span-4 space-y-4">
          ...
          <div className="bg-white p-4 rounded-2xl border border-red-100 shadow-xs">
            <div className="flex items-center justify-between border-b border-red-50 pb-3 mb-3">
            ...
            {cart.length === 0 ? (
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                {cart.map((item, idx) => (
                  <div key={item.itemId} className="bg-red-50/30 rounded-xl p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <p className="font-sans font-semibold text-[11px] text-gray-900 truncate">{item.name}</p>
                        <p className="font-mono text-[10px] text-amber-600">₹{item.price} each</p>
                      </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="w-6 h-6 rounded-lg bg-white border border-red-200 flex items-center justify-center hover:bg-red-50 cursor-pointer"
                        >
                    ...
                    {activeSpecialIdx === idx ? (
                      <div className="flex gap-1">
                        <input
                          ...
                          className="flex-1 text-[10px] px-2 py-1 border border-red-200 rounded-lg outline-none"
                          autoFocus
                        />
                        <button onClick={() => setActiveSpecialIdx(null)} className="text-amber-600 text-[10px] font-bold cursor-pointer">OK</button>
                      </div>
                    ) : (
                      <button
                        onClick={() => { setActiveSpecialIdx(idx); setSpecialText(item.specialInstructions || ''); }}
                        className="text-[9px] text-amber-500 hover:text-red-800 flex items-center gap-1 cursor-pointer"
                      >
          ...
          <div className="bg-white p-4 rounded-2xl border border-red-100 shadow-xs space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <input type="text" placeholder="Name" value={customerName} onChange={e => setCustomerName(e.target.value)}
                className="bg-red-50/30 text-xs px-3 py-2 border border-red-100 rounded-xl focus:outline-none focus:ring-1 focus:ring-amber-400 font-sans" />
            ...
            <textarea placeholder="Notes..." value={customerNotes} onChange={e => setCustomerNotes(e.target.value)} rows={1}
              className="w-full bg-red-50/30 text-xs px-3 py-2 border border-red-100 rounded-xl focus:outline-none focus:ring-1 focus:ring-amber-400 font-sans resize-none" />
          </div>
          ...
          <div className="bg-white p-4 rounded-2xl border border-red-100 shadow-xs space-y-3">
                  ...
                  return (
                    <button
                      ...
                      className={`py-2 rounded-xl text-[10px] font-bold transition-all flex flex-col items-center gap-1 cursor-pointer ${
                        paymentMethod === m.id ? 'bg-red-100 text-red-800 ring-2 ring-amber-400' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'
                      }`}
                    >
            ...
            {paymentStatus === 'pending' && (
              <div className="space-y-2">
                <input type="text" placeholder="Reason (e.g. will pay after lab)" value={pendingReason} onChange={e => setPendingReason(e.target.value)}
                  className="w-full bg-red-50/30 text-xs px-3 py-2 border border-red-100 rounded-xl focus:outline-none focus:ring-1 focus:ring-amber-400 font-sans" />
            ...
            {showQRPayment && paymentMethod === 'upi' && qrDataUrl && (
              <div className="bg-red-50 rounded-xl p-4 text-center space-y-2 border border-red-200">
                <p className="text-[10px] font-bold text-red-800">Scan to Pay ₹{grandTotal.toFixed(2)}</p>
          ...
          <div className="bg-white p-4 rounded-2xl border border-red-100 shadow-xs space-y-3">
            <div className="space-y-2 text-xs font-sans">
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <input
                    ...
                    className="w-16 text-right text-[10px] bg-red-50/50 border border-red-100 rounded px-1.5 py-0.5 outline-none"
                    min={0}
                  />
                </div>
              </div>
              ...
              <div className="border-t border-red-100 pt-2 flex justify-between font-bold text-sm">
                <span>Grand Total</span>
                <span className="font-mono text-red-800 text-base">₹{grandTotal.toFixed(2)}</span>
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full py-3.5 rounded-xl text-xs font-bold tracking-wide transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer font-display ${
                cart.length === 0 || isProcessing
                  ...
                  : 'bg-gradient-to-r from-amber-600 to-red-600 hover:from-amber-700 hover:to-red-700 text-white active:scale-98'
              }`}
            >
      ...
      {showReceipt && lastBill && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full max-h-[90vh] flex flex-col border border-red-100">
            <div className="p-6 text-center border-b border-red-50">
            ...
            <div ref={printRef} className="p-6 font-sans text-xs overflow-y-auto flex-1 min-h-0">
              ...
              <div className="text-center mt-4">
                ...
                <p className="text-[9px] text-gray-300">For support: Bite &amp; Byte Helpdesk</p>
              </div>
            </div>

            <div className="p-4 border-t border-red-50 flex gap-2 no-print shrink-0">
              <button onClick={handlePrint} className="flex-1 bg-amber-600 hover:bg-amber-700 text-white py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 cursor-pointer">
